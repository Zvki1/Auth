...
const Header = ({isButtonDisabled,nameOfGroup,membersToAdd}) => {
  ...
  const handleClick = () => {
   axios.post("http://localhost:8000/publicGroup",{name:nameOfGroup,members:membersToAdd.map((member)=>member.id)},{headers: 
   {
  ...
  return (
    <div className="w-full flex items-center justify-between px-5 py-4 border-b-2 ">
      ...
      <button disabled={isButtonDisabled} onClick={handleClick} className={`font-lato text-xl font-[600] ${isButtonDisabled?"text-[#2B363B]":" text-blue-700"}`}>
        Créer
      </button>
    </div>
  );
